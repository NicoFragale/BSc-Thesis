...
def get_data_from_table(db_connection, table, offset, limit):
    ...
    """
    cursor = db_connection.cursor()
    cursor.execute(f"SELECT user_id, username, bot, url, followers, following, statuses, description FROM {table} WHERE statuses > 200 ORDER BY statuses ASC LIMIT {limit} OFFSET {offset}")
    return cursor.fetchall()
