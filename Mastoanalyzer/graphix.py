...
def plot_metric_comparison(user_data, bot_data, metric_name, ax, user_color, bot_color, ylabel, median_user, median_bot, mean_user, mean_bot):
    """
    ...
    sns.kdeplot(user_data, color=user_color, fill=True, alpha=0.5, label=f'{metric_name} Users', ax=ax)
    sns.kdeplot(bot_data, color=bot_color, fill=True, alpha=0.3, linestyle='--', label=f'{metric_name} Bots', ax=ax)
...
def main_graphix_user(db_connection): 
    ...
    fig, axes = plt.subplots(5, 1, figsize=(10, 25))  # Added the fifth graph
    user_color = '#1f77b4'  # Blue for real users
    bot_color = '#d2b48c'   # Sand for bots
    ...
    plot_metric_comparison(usernameUser, usernameBot, "Username length", axes[0], user_color, bot_color, "Length", median_len_user, median_len_bot, mean_len_user, mean_len_bot)
    plot_metric_comparison(followersUser, followersBot, "Number of Followers", axes[1], user_color, bot_color, "Followers", median_followers_user, median_followers_bot, mean_followers_user, mean_followers_bot)
    plot_metric_comparison(followingUser, followingBot, "Number of Following", axes[2], user_color, bot_color, "Following", median_following_user, median_following_bot, mean_following_user, mean_following_bot)
    plot_metric_comparison(statusesUser, statusesBot, "Number of Statuses", axes[3], user_color, bot_color, "Statuses", median_statuses_user, median_statuses_bot, mean_statuses_user, mean_statuses_bot)
    plot_metric_comparison(descriptionUser, descriptionBot, "Description Length", axes[4], user_color, bot_color, "Description Length", median_description_user, median_description_bot, mean_description_user, mean_description_bot)
...
def main_graphix_post(db_connection): 
    ...
    fig, axes = plt.subplots(4, 1, figsize=(10, 25))  # Added the fifth graph
    user_color = '#1f77b4'  # Blue for real users
    bot_color = '#d2b48c'   # Sand for bots

    # Generate the plots, one at a time. To plot multiple graphs, change the number in line 150 to (x)
    plot_metric_comparison(timePubUser, timePubBot, "Posting Interval", axes[0], user_color, bot_color, "Seconds", median_timePubUser, median_timePubBot, mean_timePubUser, mean_timePubBot)
...
def tempo_di_risposta(array):
    ...
    plt.plot(x, array, label="Response time", color='b', linestyle='-', marker=' ') 

    # Add title and axis labels
    plt.title('Increase in Response Time', fontsize=18)
...
def plot_user_stats(db_connection):
    ...
    num_users = list(range(1, len(real_users) + 1))
