...
def get_data_from_table(db_connection, table):
    """
    ...
    """
    cursor = db_connection.cursor()
    cursor.execute(f"SELECT user_id, username, bot, url, followers, following, statuses, description FROM {table} WHERE statuses > 200 ORDER BY statuses DESC")
    return cursor.fetchall()
